...
struct SuffixAutomaton {
	vi len, lnk, lab;
	vector<map<char,int>> nex;
	vector<vi> iLnk;
	SuffixAutomaton() { len.pb(0); lnk.pb(-1); nex.eb(); lab.pb(-1); }
	int add(int p, char c) {
		auto redirect = [&]() {
			if (p == -1) return 0;
			int q = nex[p][c]; if (len[p]+1 == len[q]) return q;
			int clone = sz(len); lnk[q] = clone;
			lnk.pb(lnk[q]), nex.pb(nex[q]), len.pb(len[p]+1); lab.pb(-1); 
			for (; ~p && nex[p][c] == q; p = lnk[p]) nex[p][c] = clone;
			return clone;
		};
		if (nex[p].count(c)) return redirect();
		int cur = sz(len); // make new state
		lnk.pb(MOD), nex.eb(), len.pb(len[p]+1), lab.pb(-1);
		for (; ~p && !nex[p].count(c); p = lnk[p]) nex[p][c] = cur;
		int x = redirect(); lnk[cur] = x; return cur;
	...
	void dfs(int x) {
		trav(t,iLnk[x]) {
			dfs(t);
			lab[x] = comb(lab[x],lab[t]);
		}
	}
	vector<bool> vis;
	void dfs2(int x) {
		if (!sz(vis)) vis.rsz(sz(len));
		if (vis[x]) return;
		vis[x] = 1; trav(t,nex[x]) dfs2(t.s);
		topo.pb(x);
	}
};
...
int main() {
	...
	assert(S.topo[0] == 0);
	dbg(S.topo);
	ways.rsz(sz(S.len)); ways[0] = 1;
	dbg("OK");
	F0R(i,sz(S.topo)) {
		int x = S.topo[i];
		trav(t,S.nex[x]) ways[t.s] += ways[x];
		if (S.lab[x] != MOD) ans[S.lab[x]] += ways[x];
	}
	F0R(i,N) ps(ans[i]);
