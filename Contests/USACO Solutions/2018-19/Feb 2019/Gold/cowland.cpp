...
int main() {
    ...
    L.init(N);
    FOR(a,1,N+1) {
        B.upd(L.resp[a].f,e[a]);
        B.upd(L.resp[a].s+1,e[a]);
    }
    F0R(i,Q) {
        int t,a,b; re(t,a,b);
        if (t == 1) {
            B.upd(L.resp[a].f,e[a]^b);
            B.upd(L.resp[a].s+1,e[a]^b);
            e[a] = b;
        } else {
            pr(B.sum(L.resp[a].f)^B.sum(L.resp[b].f)^e[L.lca(a,b)],"\n");
        }
    }
}
