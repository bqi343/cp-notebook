...
Seg<double,1<<20> S;
int N;
ld p[MX],ans,cum[MX], eps=1e-12;

ld eval(int l, int r) {
    if (l > r) return 0;
    // ps(l,r,cum[r+1]-cum[l],S.query(l,r));
    return (cum[r+1]-cum[l])*S.query(l,r);
}
...
int main() {
    ...
    F0R(i,N) {
        ...
        // p[i] = rand() % 999999+1;
        p[i] = 1-p[i]/1000000;
        cum[i+1] = cum[i]+1/p[i]-1+eps;
        S.upd(i,p[i]);
    }
    int r = -1;
    F0R(l,N) {
        while (r < N-1 && cum[r+1]-cum[l] <= 1) r ++;
        // ps("HUH",l,r,sum);
        ckmax(ans,eval(l,r-1));
        ckmax(ans,eval(l,r));
    }
    pr((ll)floor(1000000*ans));
    // you should actually read the stuff at the bottom
}
