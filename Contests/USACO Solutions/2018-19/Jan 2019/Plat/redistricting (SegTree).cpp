...
multiset<int> bes[MX];
int N,K,dp[MX],cur[MX];

void ins(int a, int b) {
    if (b == 1) bes[cur[a]].insert(dp[a]);
    else bes[cur[a]].erase(bes[cur[a]].find(dp[a]));
    if (sz(bes[cur[a]])) S.upd(cur[a],*bes[cur[a]].begin());
    else S.upd(cur[a],MOD);
}
...
int main() {
    ...
    cur[0] = 1<<19; ins(0,1);
    FOR(i,1,N+1) {
        if (i-K-1 >= 0) ins(i-K-1,-1);
        cur[i] = cur[i-1]; 
        char c; re(c);
        if (c == 'H') cur[i] ++;
        else cur[i] --; 
        
        dp[i] = min(S.query(0,cur[i]-1),S.query(cur[i],MX-1)+1);
        ins(i,1);
        // < cur 
        // >= cur + 1 
