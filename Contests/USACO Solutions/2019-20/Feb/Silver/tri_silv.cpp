...
void check() {
	F0R(i,20001) if (sz(todo[i])) {
		sort(all(todo[i]));
		mi cur = 0; 
		F0R(j,sz(todo[i])) 
			cur += todo[i][j].f-todo[i][0].f;
		F0R(j,sz(todo[i])) {
			if (j) cur += (2*j-sz(todo[i]))*(todo[i][j].f-todo[i][j-1].f);
			// sz(todo[i])-j, j
...
int main() {
	...
	F0R(i,20001) todo[i].clear();
	F0R(i,sz(v)) todo[v[i].f+10000].pb({v[i].s,i});
	check();
	F0R(i,20001) todo[i].clear();
	F0R(i,sz(v)) todo[v[i].s+10000].pb({v[i].f,i});
	check();
	mi ans = 0;
	F0R(i,N) ans += sum[i][0]*sum[i][1];
	ps(ans);
