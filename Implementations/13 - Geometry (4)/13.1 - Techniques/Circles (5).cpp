...
namespace circle {
	...
	double arcLength(circ x, P a, P b) {
	    P d = (a-x.f)/(b-x.f);
	    return x.s*acos(d.f);
	}
	...
	P intersect(circ x, circ y, int t = 0) { 
	    double d = abs(x.f-y.f); // distance between centers
	    double theta = acos((x.s*x.s+d*d-y.s*y.s)/(2*x.s*d)); // law of cosines
	    P tmp = (y.f-x.f)/d*x.s;
	    return x.f+tmp*cis(t == 0 ? theta : -theta);
	}
	...
	P tangent(P x, circ y, int t = 0) {
	    ...
	    P b = sqrt(d*d-y.s*y.s)/d*y.s*dir(x-y.f)*cis(PI/2); 
	    return t == 0 ? a+b : a-b;
	}
	...
	vector<pair<P,P>> external(circ x, circ y) { // external tangents
	    vector<pair<P,P>> v; 
	    if (x.s == y.s) {
	        P tmp = dir(x.f-y.f)*x.s*cis(PI/2);
