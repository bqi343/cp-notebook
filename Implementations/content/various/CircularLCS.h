...
int circular_lcs(str A, str B) { // returns max LCS over all rotations
	...
	F0R(j,sz(B)/2) { // 1. zero out dp[.][j], update dif_left and dif_right
		...
		int cur_lcs = 0; // 2. calculate LCS(A[0:sz(A)),B[j:j+sz(B)/2))
